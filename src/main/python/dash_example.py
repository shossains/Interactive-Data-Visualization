...
def update_graph(contents, filename, xvalue, yvalue, charvalue, plotvalue):
    """
    ...
    if df is not None:
        ...
        if('scatter' in plotvalue):
            fig = px.scatter(df, x=x, y=y, color=charvalue, hover_data=df)

        if('density' in plotvalue):
            fig.add_histogram2d(x=x, y=y, colorscale='Blues', name='density', showlegend=False, showscale=False)
